...
library = [
  ...
  { titre: 'live', nom: 'lisa', prix: 120, quantite: 22 },
];
...
function recherche(library) {
  ...
  //    if(found) console.log("le live exist .");
  //    else console.log("le live ne exist pas .");
  found
    ? console.log('le live exist .')
    : console.log('le live ne exist pas .');
}
function mettreajourquantite(library) {
  ...
  index = -1;
  for (let i = 0; i < library.length; i++) {
    if (library[i].titre.toUpperCase() == titre.toUpperCase()) {
      index = i;
      break;
    }
  }
  if (index == -1) {
    ...
    console.log(
      'La quantité actuelle de ce livre est :',
      library[index].quantite,
    );

    let quantite = prompt("s'il te entrer  la  nouvelle quantité ");
  ...
  if (index == -1) {
    ...
    for (let i = 0; i < library.length; i++) {
      if (library[i] == library[index]) {
        ...
        //   newlibrary[newlibrary.length] = "harry";
      }
    }
  }
  library = newlibrary;
...
function stock(library) {
  let stock = 0;
  for (let i = 0; i < library.length; i++) {
    stock += library[i].quantite;
  }
  console.log(stock);
}
